"""
    Keypoint Detection 
        ...
        checkpoints written to ~/data/ubt/checkpoints/$project_name$/keypoints
...
def train(config, log_images = True):
    ...
    if not 'experiment' in config:
        checkpoint_path = os.path.expanduser("~/checkpoints/ubt/keypoints/vids%s-%s" % (config['train_video_ids'],now))
