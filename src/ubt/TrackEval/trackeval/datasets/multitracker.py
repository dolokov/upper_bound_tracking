...
class Multitracker(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self.output_fol = os.path.expanduser('~/data/ubt/evaluation')
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        csv_gt = self.config['_csv_trackannotation']  #'/home/alex/data/ubt/projects/1/1/trackannotation.csv'
        csv_tracked = self.config['_csv_tracked'] #'/home/alex/data/ubt/projects/1/tracking_Tracking_under_occlusion_UpperBound_ssd_2stack_none_2020-11-25_08-47-15_22772819_rec-00.00.00.000-00.10.20.916-seg1.csv'
        ...
        with open(csv_gt, 'r') as f:
            ...
            header_gt = lines_gt[0]
            lines_gt = lines_gt[1:]
            lines_gt = [l.replace('\n','').split(',') for l in lines_gt]
            cnt_gt = 0 
            for [frame_idx,idv,x1,y1,x2,y2] in lines_gt:
                if cnt_gt > 0: # skip header
        ...
        with open(csv_tracked, 'r') as f:
            lines_tracked = f.readlines()
            header_tracked = lines_tracked[0]
            lines_tracked = lines_tracked[1:]
            lines_tracked = [l.replace('\n','').split(',') for l in lines_tracked]
            for [video_id,frame_idx,idv,centerx,centery,x1,y1,w,h,time_since_update] in lines_tracked:
                ...
                frame_idx,idv = int(frame_idx),int(idv)
                if frame_idx >= 0:
                    if frame_idx in raw_data['tracker_ids']:
        ...
        print('[*] num tracker ids', len(raw_data['tracker_ids'].keys()))
        print('[*] num GT ids', len(raw_data['gt_ids'].keys()))
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if 0:
                ...
                if t < len(raw_data['gt_ids']):
                    ...
                    #print(t,'gt_ids',gt_ids,'gt_dets',gt_dets,'tracker_ids',tracker_ids,'tracker_dets',tracker_dets)
